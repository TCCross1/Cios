...
| `pnpm arch:check`   | Validate `@cios/*` package-manifest dependencies (`dependencies`, `devDependencies`, `optionalDependencies`, `peerDependencies`) against `docs/architecture/dependency-policy.json`; fails closed on malformed manifests and on policy/workspace drift |
